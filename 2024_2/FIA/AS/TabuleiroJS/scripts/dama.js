...
function initializeBoard() {
	var initialBoard = [[white, empty, white, empty, white, empty, white, empty],
	...
	return { cells: cells, pieces: pieces, turn: white };
}
...
function novoJogo(origin, cellWidth, tabuleiro) {
	...
	board_origin = origin;
	currentBoard = drawBoard(origin, cellWidth, tabuleiro);
...
function replayAll(origin, cellWidth, tabuleiro) {
	var allMoves = movePiece.moves;
	novoJogo(origin, cellWidth, tabuleiro);
...
function getPieceCount(boardState) {
	...
	return {
		white: numWhite , black: numBlack
	};
}

function getScore(boardState) {
	var pieceCount = getPieceCount(boardState);
	var score = pieceCount.white - pieceCount.black;
	return score;
}
...
function dragged(d) {
	...
	//MODIFICADO
	highlightLegalMoves(currentBoard.cells, d);
...
function getJumpedPiece(cells, pieces, from, to) {
	...
	for (var i = 0; i < pieces.length; i++) {
		if (pieces[i].col === midCol && pieces[i].row === midRow) {
			return pieces[i];
		}
	}
	return null;
...
function isMoveLegal(cells, pieces, piece, from, to) {
	...
	if ((Math.abs(piece.state) !== 1.1) && (abs(distance.x) > 2)) {
		return false;
	}
	...
	*/
	...
	if (
		(currentBoard.turn === 1 && piece.state !== 1 && piece.state !== 1.1) ||  // Turno das brancas
		(currentBoard.turn === -1 && piece.state !== -1 && piece.state !== -1.1) // Turno das pretas
	) {
		return false;
	}
...
function drawBoard(origin, cellWidth, tabuleiro) {
	...
	tabuleiro.append("g")
		...
		.data(cells)
		.enter().append("rect")
		.attr("id", function (d) { return `cell-${d.row}-${d.col}`; })
		...
		.style("fill", function (d) {
			...
			if ((d.row + d.col) % 2 === 1) return "white";
			else return "black";
		})
		.style("stroke", "rgb(0, 0, 0)")
		.style("stroke-width", "2px");

	// Função de arrastar para as peças
	...
	tabuleiro.append("g")
		...
		.data(pieces)
		.enter().append("circle")
		.attr("r", cellWidth / 2.5) // Ajuste para um tamanho menor
		.attr("cx", function (d) {
...
function drawText(data) {
	tabuleiro.append("g")
		...
		.style("font-size", function (d) { 
            if (d.state === whiteKing || d.state === blackKing) return cell_width * 0.3 + "px"; // Ajusta o tamanho relativo à célula
            else return "0"; // Outros estados não têm texto
        })
		.style("fill", function (d) { 
            if (d.state === white) return "black"; 
            else return "white"; 
        })
		.text(function (d) { 
            if (d.state === whiteKing || d.state === blackKing) return "👑"; // Exibe a coroa apenas para damas
            else return ""; 
        });
}
...
function getLegalMoves(boardCells, pieces, piece) {
	const moves = [];
	const directions = [
		{ row: -1, col: -1 },
		{ row: -1, col: 1 },
	...
	directions.forEach(direction => {
		const targetRow = piece.row + direction.row;
		const targetCol = piece.col + direction.col;

		if (targetRow >= 0 && targetRow < 8 && targetCol >= 0 && targetCol < 8) {
			const targetCell = boardCells.find(cell => cell.row === targetRow && cell.col === targetCol);
			if (targetCell && isMoveLegal(boardCells, pieces, piece, piece, targetCell)) {
...
function highlightLegalMoves(cells, piece) {
	// Limpa os destaques anteriores
	d3.selectAll("rect").style("stroke", "black").style("stroke-width", "2px");

	cells.forEach(cell => {
		// Verifica se o movimento é legal
		if (isMoveLegal(cells, currentBoard.pieces, piece, piece, cell)) {
			// Adiciona destaque à célula
			d3.select(`#cell-${cell.row}-${cell.col}`)
				.style("stroke", "yellow")
